...
impl DropLogfileGroupStatement {
    pub fn parse(i: &str) -> IResult<&str, DropLogfileGroupStatement, ParseSQLError<&str>> {
        ...
        let (remaining_input, (_, _, _, _, _, _, logfile_group, _, engine_name, _)) = parser(i)?;
...
mod tests {
    ...
    fn parse_drop_logfile_group_parser() {
        let sqls = [
            "DROP LOGFILE GROUP logfile_group ENGINE = demo;",
            "DROP LOGFILE GROUP logfile_group ENGINE bar;",
        ];
        let exp_statements = [
            DropLogfileGroupStatement {

...
/// DROP [UNDO] TABLESPACE tablespace_name
///     [ENGINE [=] engine_name]
pub fn drop_tablespace(i: &str) -> IResult<&str, DropTablespaceStatement, VerboseError<&str>> {
...
#[cfg(test)]
mod tests {
    use dds::drop_tablespace::drop_tablespace;
    ...
    fn test_drop_tablespace_parser() {
        ...
        for i in 0..sqls.len() {
            ...
            let res = drop_tablespace(sqls[i]);
            // res.unwrap();
            println!("{:?}", res);
            // assert!(res.is_ok());
            // println!("{:?}", res);

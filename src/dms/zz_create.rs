...
pub fn field_specification(i: &str) -> IResult<&str, ColumnSpecification, VerboseError<&str>> {
    let (remaining_input, (column, field_type, constraints, comment, _)) = tuple((
        ...
        opt(delimited(multispace1, DataType::type_identifier, multispace0)),
...
mod tests {
    ...
    fn sql_types() {
        ...
        let res = DataType::type_identifier(type0);
        assert_eq!(res.unwrap().1, DataType::Bigint(20));
        let res = DataType::type_identifier(type1);
        assert_eq!(res.unwrap().1, DataType::Varchar(255));
        let res = DataType::type_identifier(type2);
        assert_eq!(res.unwrap().1, DataType::UnsignedBigint(20));
        let res = DataType::type_identifier(type3);
        assert_eq!(res.unwrap().1, DataType::Bigint(20));
        let res = DataType::type_identifier(type2);
        assert_eq!(res.unwrap().1, DataType::UnsignedBigint(20));
    }
    ...
    fn field_spec() {
        ...
        let str = "id bigint(20), name varchar(255),";

        let res = field_specification_list(str);
        assert_eq!(
            res.unwrap().1,
            vec![
                ColumnSpecification::new(Column::from("id"), DataType::Bigint(20)),
                ColumnSpecification::new(Column::from("name"), DataType::Varchar(255)),
            ]
        );
    }
    ...
    #[test]
    fn simple_create() {
        let str = "CREATE TABLE users (id bigint(20), name varchar(255), email varchar(255));";

        let res = creation(str);
        assert_eq!(
            res.unwrap().1,
            CreateTableStatement {
                table: Table::from("users"),
                fields: vec![
                    ColumnSpecification::new(Column::from("users.id"), DataType::Bigint(20)),
                    ColumnSpecification::new(Column::from("users.name"), DataType::Varchar(255)),
    ...
    fn create_without_space_after_tablename() {
        let str = "CREATE TABLE t(x integer);";
        let res = creation(str);
        assert_eq!(
            res.unwrap().1,
            CreateTableStatement {
    ...
    fn create_tablename_with_schema() {
        let str = "CREATE TABLE db1.t(x integer);";
        let res = creation(str);
        assert_eq!(
            res.unwrap().1,
            CreateTableStatement {
    ...
    #[test]
    fn mediawiki_create() {
        let str = "CREATE TABLE user_newtalk (  user_id int(5) NOT NULL default '0',  user_ip \
    ...
    fn mediawiki_create2() {
        let str = "CREATE TABLE `user` (
                        user_id int unsigned NOT NULL PRIMARY KEY AutoIncrement,
    ...
    fn mediawiki_create3() {
        let str = "CREATE TABLE `interwiki` (
 ...
 iw_trans tinyint NOT NULL default 0
    ...
    fn mediawiki_externallinks() {
        let str = "CREATE TABLE `externallinks` (
          `el_id` int(10) unsigned NOT NULL AutoIncrement,
    ...
    #[test]
    fn django_create() {
        let str = "CREATE TABLE `django_admin_log` (
        ...
        let str = "CREATE TABLE `auth_group` (
                       `id` integer AutoIncrement NOT NULL PRIMARY KEY,
    ...
    fn format_create() {
        ...
        let expected = "CREATE TABLE auth_group (\
                        id INT(32) AutoIncrement NOT NULL PRIMARY KEY, \
                        name VARCHAR(80) NOT NULL UNIQUE)";
        let res = creation(str);
    ...
    fn simple_create_view() {
        let str = "CREATE VIEW v AS SELECT * FROM users WHERE username = \"bob\";";

        let res = view_creation(str);
        assert_eq!(
            res.unwrap().1,
            CreateViewStatement {
                name: String::from("v"),
    ...
    fn compound_create_view() {
        let str = "CREATE VIEW v AS SELECT * FROM users UNION SELECT * FROM old_users;";
    ...
    fn format_create_view() {
        let str = "CREATE VIEW `v` AS SELECT * FROM `t`;";
        ...
        assert_eq!(
            ...
            CreateTableStatement {
                ...
                fields: vec![
                    ...
                    ColumnSpecification::new(Column::from("comments.hat_id"), DataType::Int(32)),
                ],
                keys: Some(vec![
